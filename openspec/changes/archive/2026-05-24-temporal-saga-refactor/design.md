...
**决策**：Activity 收到账户服务返回的确定性业务失败时，应先将 `transfer_order` 更新为对应失败终态并记录错误上下文，再抛出 `ApplicationFailure.newNonRetryableFailure()`；网络超时、服务不可用等 Feign 异常继续上浮给 Temporal RetryPolicy 重试。

**理由**：业务失败与瞬时失败的处理语义不同。余额不足、账户不存在等失败重复执行仍会失败，等待 Temporal 重试耗尽会让查询侧长期看到 `CREATED`，无法区分"刚提交"和"冻结失败"。使用非重试 ApplicationFailure 可让 Workflow 立即进入 FAILED，同时 DB 侧保留可查询的业务终态。
...
- `transfer_order` 的 `status` 字段在 Activity 失败重试期间是否需要体现"重试中"状态？（不需要为瞬时失败暴露"重试中"状态；但确定性业务终态失败必须立即写入 DB，例如冻结业务失败写入 `FREEZE_FAILED`，并通过 `ApplicationFailure.newNonRetryableFailure()` 阻止无意义重试）
